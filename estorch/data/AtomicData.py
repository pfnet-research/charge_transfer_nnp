...
class AtomicData(Data):
    ...
    def __init__(self, irreps: Dict[str, e3nn.o3.Irreps] = {}, **kwargs):
        ...
        for k, v in kwargs.items():
            if (
                ...
                kwargs[k] = torch.as_tensor(v, dtype=torch.get_default_dtype()).unsqueeze(-1)
        ...
        if AtomicDataDict.ATOMIC_NUMBERS_KEY in self and self.atomic_numbers is not None:
        ...
        if atoms.calc is not None:
            if isinstance(atoms.calc, (SinglePointCalculator, SinglePointDFTCalculator)):
                ...
                if "free_energy" in add_fields and "energy" not in add_fields:
                    add_fields[AtomicDataDict.TOTAL_ENERGY_KEY] = add_fields.pop("free_energy")
                elif "energy" in add_fields:
                    add_fields[AtomicDataDict.TOTAL_ENERGY_KEY] = add_fields.pop("energy")
    ...
    @staticmethod
    def to_AtomicDataDict(data: Union[Data, Mapping], exclude_keys=tuple()) -> AtomicDataDict.Type:
        ...
        return {
            ...
            for k in keys
            if (
                k not in exclude_keys and data[k] is not None and isinstance(data[k], torch.Tensor)
            )
        }
    ...
    def __cat_dim__(self, key, value):
        if key in (
            ...
            TOTAL_CHARGE_KEY,  # total_charge is global property
        ):
    ...
    def without_nodes(self, which_nodes):
        ...
        for k in self.keys:
            if k == AtomicDataDict.EDGE_INDEX_KEY:
                new_dict[AtomicDataDict.EDGE_INDEX_KEY] = new_index[self.edge_index[:, edge_mask]]
            elif k == AtomicDataDict.EDGE_CELL_SHIFT_KEY:
                new_dict[AtomicDataDict.EDGE_CELL_SHIFT_KEY] = self.edge_cell_shift[edge_mask]
...
def neighbor_list_and_relative_vec(
    ...
    # Build output:
    edge_index = torch.vstack((torch.LongTensor(first_idex), torch.LongTensor(second_idex))).to(
        device=out_device
    )
