...
from estorch.data import AtomicData
...
class AtomicInMemoryDataset(AtomicDataset):
    ...
    def __init__(
        ...
        force_fixed_keys = set(force_fixed_keys).union(getattr(type(self), "FORCE_FIXED_KEYS", []))
        ...
        if self.data is None:
            self.data, self.fixed_fields, include_frames = torch.load(self.processed_paths[0])
        ...
        for key, value in fixed_fields.items():
            if isinstance(value, np.ndarray):
                if np.issubdtype(value.dtype, np.floating):
                    fixed_fields[key] = torch.as_tensor(value, dtype=torch.get_default_dtype())
                else:
    ...
    def statistics(
        ...
        node_selector = torch.as_tensor(np.in1d(self.data.batch.numpy(), selector.numpy()))
        ...
        for ifield, field in enumerate(fields):
            ...
            if ana_mode == "count":
                uniq, counts = torch.unique(torch.flatten(arr), return_counts=True, sorted=True)
...
class NpzDataset(AtomicInMemoryDataset):
    ...
    def get_data(self):
        ...
        fixed_fields = {k: v for k, v in mapped.items() if k in self.npz_fixed_field_keys}
